...
# allocate memory for aligned volume  that is slightly larger than original volume due to max. shifts.
...
dvol = np.diff(aligned_vol,axis=1) # the 1st dimension is depth
...
try:
    ...
    if False:
        ...
        # Create a vector that is the same size as the  aligned volume.
...
# Do the flattening of volume and save it to the new aligned_vol
for k in range(vol.shape[0]):
    print(dz[k])
...
try:
    ...
    # This is that zero trick again before saving the values and creating the aligned_vol for the last time.
    fitted_z = -fitted_z
    dz = np.round(fitted_z).astype(np.int)
    dz = dz-dz.min()
