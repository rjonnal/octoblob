...
phase_variance_png_contrast_percentiles = cfg.phase_variance_png_contrast_percentiles
amplitude_variance_png_contrast_percentiles = cfg.amplitude_variance_png_contrast_percentiles
...
def save_angiogram(tup,diagnostics=False):
    # this function takes a tuple:
    # tup[0] is a list of filenames corresponding to bm_scans
    # tup[1] is an output index
    # tup[2] is an amplitude output folder
    # tup[3] is an amplitude variance output folder
    # tup[2] is an phase variance output folder
    # the function loads the B-scans, computes an angiogram, and saves the resulting angiogram
    #print(tup)

    block_filenames = tup[0]
    output_index = tup[1]
    amplitude_output_folder = tup[2]
    amplitude_variance_output_folder = tup[3]
    phase_variance_output_folder = tup[4]
    ...
    amplitude_fn = os.path.join(amplitude_folder,'amp_%05d.npy'%output_index)
    amplitude_variance_fn = os.path.join(amplitude_variance_folder,'av_%05d.npy'%output_index)
    phase_variance_fn = os.path.join(phase_variance_folder,'pv_%05d.npy'%output_index)

    np.save(amplitude_fn,amplitude)
    np.save(phase_variance_fn,phase_variance)
    np.save(amplitude_variance_fn,amplitude_variance)
...
if __name__=='__main__':
    ...
    success_log_filename = 'process_octa_successes.log'
    failure_log_filename = 'process_octa_failures.log'
    ...
    for fn in files:
        ...
        n_bm_scans = cfg['n_bm_scans']
        try:
            assert cfg['n_slow']==len(bscan_files)
        except AssertionError as ae:
            with open(failure_log_filename,'a') as fid:
                fid.write('[%s] Mismatch between number of bscans (%d) and xml file expectation (%d).\n'%(datetime.datetime.now(),len(bscan_files),cfg['n_slow']))
                continue
            

        octa_folder = os.path.join(bscan_folder,'octa')
        os.makedirs(octa_folder,exist_ok=True)
        amplitude_folder = os.path.join(octa_folder,'amplitude')
        phase_variance_folder = os.path.join(octa_folder,'phase_variance')
        amplitude_variance_folder = os.path.join(octa_folder,'amplitude_variance')

        os.makedirs(amplitude_folder,exist_ok=True)
        os.makedirs(phase_variance_folder,exist_ok=True)
        os.makedirs(amplitude_variance_folder,exist_ok=True)
        ...
        N = len(bscan_files)
        first_start = 0
        last_start = N-n_bm_scans

        start_indices = range(first_start,last_start+n_bm_scans,n_bm_scans)
        if len(glob.glob(os.path.join(amplitude_variance_folder,'*.npy')))>=len(start_indices):
            print('%s done. Skipping.'%amplitude_variance_folder)
            continue
        block_filenames = []
        for si in start_indices:
            filenames = bscan_files[si:si+n_bm_scans]
            block_filenames.append(filenames)

        output_indices = range(0,len(block_filenames))
        output_folders = [octa_folder]*len(output_indices)

        tups = []
        for k in range(0,len(block_filenames)):
            tups.append((block_filenames[k],k,amplitude_folder,amplitude_variance_folder,phase_variance_folder))

        #tups = tups[:4]

        try:
            p = mp.Pool(n_cpus)
            success = p.map(save_angiogram,tups)
            ...
            success = []
            for tup in tups:
                success.append(save_angiogram(tup))
