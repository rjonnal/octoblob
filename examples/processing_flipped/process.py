...
# Decide if you want to see the bscans--this slows things down but may be useful for debugging
# It may make optimization very slow, since many iterations are involved.
show_bscans = True
if show_bscans:
    bscan_figure = plt.figure()
    bscan_figure.suptitle('Processed B-scan')
    bscan_figure.add_subplot(1,1,1)
...
def spectra_to_bscan(mdcoefs,spectra,diagnostics=None):
    ...
    # The retina is located near the bottom of the remaining B-scan, so remove
    # the top 500 pixels/rows
    bscan = bscan[500:,:]

    
    if show_bscans:
        plt.figure(bscan_figure.number)
        ax = bscan_figure.axes[0]
        ax.clear()
        ax.imshow(20*np.log10(np.abs(bscan)),clim=(40,80),cmap='gray')
        plt.pause(.1)
...
def spectra_to_bscan_for_optimization(mdcoefs,spectra,diagnostics=None):
    # this is a convenience function in case the spectra or bscan need to be processed
    # only in the case of optimization

    # do some spectra processing, e.g. cropping, etc
    #spectra = spectra[:,:10] # use only the first 50 A-scans in the B-scan for optimization, to speed things up

    # do the main OCT conversion
    bscan = spectra_to_bscan(mdcoefs,spectra,diagnostics=diagnostics)

    # do some postprocessing of the bscan, e.g. cropping, etc.
    return bscan
...
try:
    ...
    samples = src.get_samples(10)
    # modify the next line to use the local spectra_to_bscan function by removing 'blobf.':
    coefs = mdo.multi_optimize(samples,spectra_to_bscan_for_optimization,show_all=False,show_final=True,verbose=False,diagnostics=diagnostics)
