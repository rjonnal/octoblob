...
kernel = np.zeros(mbscan.shape)
kernel[:1,:17] = 1.0
mbscan = 20*np.log10(mbscan)
mbscan0 = mbscan
for k in range(1):
    mbscan = np.real(np.fft.ifft2(np.fft.fft2(mbscan)*np.fft.fft2(kernel)))
plt.figure()
plt.imshow(mbscan)
plt.figure()
plt.imshow(mbscan0)
...
mbscan_segments = slic(mbscan,
                          n_segments=10,
                          compactness=1,channel_axis=None)
...
plt.imshow(mark_boundaries(mbscan, mbscan_segments))
plt.show()
sys.exit()
