...
if __name__=='__main__':
    ...
    def onclick(event):
        global counter,last_x,pv_vol,top_vec,bottom_vec,plot_ax,org_all
        global tops_all,bottoms_all,img_ax,x_tops,y_tops,x_bottoms,y_bottoms,profile_ax
        if event.inaxes==img_ax:
            if counter%2==0:
                ...
                profile_ax.clear()
                profile_ax.plot(prof,z)

                tx,ty = top_vec[-1]
                profile_ax.plot(prof[ty],ty,'ro')
                profile_ax.set_ylim(max(profile_ax.get_ylim()), min(profile_ax.get_ylim()))
                plt.pause(0.001)
