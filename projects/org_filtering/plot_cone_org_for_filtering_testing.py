...
def plot(folder,stim_index=100):
    ...
    ax3.axvline(0.0,color='g',linestyle='--')
    plt.pause(.0001)
    ...
    def draw_rois():
        ...
        for k,roi in enumerate(rois):
            bx1,bx2 = [a[0] for a in roi[0][:2]]
            bz1,bz2 = [a[1] for a in roi[0][:2]]
            bx1-=1
            bx2+=1
            bz1-=1
            bz2+=1
            ...
            isos_z = roi[3][stim_index,:]
            cost_z = roi[4][stim_index,:]
            
            ax1.plot(x,isos_z,color=colors[k%len(colors)],alpha=line_alpha,linewidth=line_linewidth)
            ax1.plot(x,cost_z,color=colors[k%len(colors)],alpha=line_alpha,linewidth=line_linewidth)
        ...
        for k,roi in enumerate(rois):
            ...
            ax3.plot(t,osa,linewidth=plot_linewidth,alpha=plot_alpha,color=colors[k%len(colors)])
            
            
        if len(rois)>1:
            osv_mat = np.array(osv_mat)
            osa_mat = np.array(osa_mat)
            mosv = np.nanmean(osv_mat,axis=0)
            mosa = np.nanmean(osa_mat,axis=0)
            
            ax2.plot(t,mosv,color='k',alpha=mplot_alpha,linewidth=mplot_linewidth)

            ax3.plot(t,mosa,color='k',alpha=mplot_alpha,linewidth=mplot_linewidth)
    ...
    def onclick(event):
        ...
        if event.button==1:
            ...
            if len(click_points)==4:
                ...
                try:
                    #osa,osv,isos_z,cost_z,x1,x2 = blobo.extract_layer_velocities_lines(abscans,pbscans,x1,x2,z1,z2,x3,x4,z3,z4,stim_index=stim_index)
                    osa,osv,isos_z,cost_z,x1,x2 = blobo.extract_layer_velocities_region(abscans,pbscans,x1,x2,z1,z2,stim_index=stim_index)
                ...
                if valid:
                    ...
                    rois.append((click_points,osa,osv,isos_z,cost_z,x1,x2))
                    click_points = []
