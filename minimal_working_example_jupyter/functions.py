...
def guess_bscan_crop_coords(bscan,padding=cfg.autocrop_padding):
    ...
    if False:
        plt.figure()
        plt.plot(mprof)
        plt.axhline(inner_thresh)
        plt.axhline(outer_thresh)
        plt.axvline(z1)
        plt.axvline(z2)
        print(inner_thresh)
        print(outer_thresh)
...
def read_looky(tag):
    ...
    if os.path.exists(lfn):
        with open(lfn,'r') as fid:
            lines = fid.readlines()
        s = lines[1].strip()
        coord_strings = [k.strip() for k in s[s.find("(")+1:s.find(")")].split(',')]
        coords = [float(p.split(' ')[0]) for p in coord_strings]
        directions = [p.split(' ')[1] for p in coord_strings]
        radius = np.sqrt(np.sum(np.array(coords)**2))
        out = {}
        out['radius'] = radius
        out['horizontal_direction'] = directions[0]
        out['vertical_direction'] = directions[1]
        out['horizontal_coord'] = coords[0]
        out['veritcal_coord'] = coords[1]
        return out
